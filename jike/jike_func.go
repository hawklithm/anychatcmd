...
type NewsFeedList struct {
	LoadMoreKey NextKey `json:"loadMoreKey"`
}

func (w *Jike) GetList() (err error) {

	apiURI := "https://app.jike.ruguoapp.com/1.0/newsFeed/list"
	result := w.Post(apiURI, nil, true)
...
func (w *Jike) getWechatRoomMember(roomID, userId string) (roomName,
...
func (w *Jike) getImg(msgId string, preview bool) (img image.Image,
	err error) {
...
func (w *Jike) getSyncMsg() (msgs []interface{}, err error) {
	//name := "webwxsync"
...
//func (w *Jike) convertMsg(msg *Message) {
...
//func (w *Jike) unmarshalHyperLinkXml(content string) *HyperLink {
//
...
//func (w *Jike) dealImageMessage(msg Message) *MessageRecord {
//	msgId := msg.MsgId
...
//func (w *Jike) getImageMessage(msg Message) *image.Image {
//	msgId := msg.MsgId
//	if img, err := w.getImg(msgId, false); err != nil {
//		w.Log.Fatalln("get image error! msgId=", msgId, err)
//		return nil
...
////同步守护goroutine
//func (w *Jike) SyncDaemon(msgIn chan Message) {
//	for {
...
//func (w *Jike) MsgDaemon(msgOut chan MessageRecord, autoReply chan int) {
//	msg := MessageRecord{}
//	var autoMode int
//	for {
//		select {
...
//func (w *Jike) StatusNotify() (err error) {
...
//func (w *Jike) GetContactsInBatch(memberNames []string) (member []Member,
//	err error) {
...
func (w *Jike) Post(url string, data url.Values, jsonFmt bool) (result string) {
...
func (w *Jike) Get(url string, data url.Values) (result string) {
	//req.Header.Set("User-agent", UserAgent)

	resp, err := w.Client.Get(url + "?" + data.Encode())
